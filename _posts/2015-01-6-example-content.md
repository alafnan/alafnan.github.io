...
<p style="text-align: justify; text-justify: inter-word;"> The data was visualised using <strong>Jupyter Notebook</strong> as shown in the figure below. The figure shows the first five rows out of a total of <i>203363</i> rows using the follwoing comman line {% highlight js %}Train.head()      // to read file Train{% endhighlight %} Notice that the data was read from file <strong>turnstile_190511</strong> on MTA website. </p>
